...
async function main() {
  try {
    ...
    let sheet_info = await sheet.getDefaultInfo();
    if (!sheet_info) {
      console.error("Can not get sheet info");
      return;
    }
    await getData(sheet_info);

    sheet.subscribe(async (sheet_info) => {
      await getData(sheet_info);
    })
  } catch (e) {
...
async function getData (sheet_info) {
  try {
